...
async def _chunked_max_size_bytes(request: Request) -> int:
    """Resolve the effective max declared size for a chunked upload.

    Precedence:
      1. CHUNKED_UPLOAD_MAX_SIZE_MB env var (if > 0)
      2. admin-configured FILE_MAX_SIZE (MB), read from the DB-backed config
         under key 'rag.file.max_size'. open-webui >= 0.10 reshaped config into
         per-key rows (Config.get_many) — there is no more app.state.config.
    ...
    configured = None
    try:
        values = await Config.get_many('rag.file.max_size')
        configured = values.get('rag.file.max_size')
    except Exception:
        configured = None
...
async def chunked_upload_start(
    ...
    max_bytes = await _chunked_max_size_bytes(request)
